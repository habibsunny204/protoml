...
with tab_ai:
    if not api_key:
        ...
        last_eng = st.session_state.last_engine
        if last_eng and st.session_state[last_eng].get("results_df") is not None:
            _eng_r = st.session_state[last_eng]
            _eng_label = {"tabular": "Tabular ML", "vision": "Vision DL",
                          "nlp": "NLP"}.get(last_eng, last_eng.title())
            st.caption(
                f"Context: last run was **{_eng_label}** — "
                f"best score {_eng_r['best_acc']}"
            )
        render_ai_chat(api_key)
